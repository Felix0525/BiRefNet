...
# Edge uses two fixed aspect-ratio-preserving sizes (picked per batch) via
# custom_collate_fn, same as the dynamic_size path; both need the original-size
# images fed to collate.
use_dynamic_collate = bool(config.dynamic_size) or (config.task == 'Edge' and getattr(config, 'edge_sizes', None))
...
def prepare_dataloader(dataset: torch.utils.data.Dataset, batch_size: int, to_be_distributed=False, is_train=True):
    ...
    if to_be_distributed:
        return torch.utils.data.DataLoader(
            ...
            shuffle=False, sampler=DistributedSampler(dataset), drop_last=True, collate_fn=custom_collate_fn if is_train and use_dynamic_collate else None
        )
    else:
        return torch.utils.data.DataLoader(
            dataset=dataset, batch_size=batch_size, num_workers=min(config.num_workers, batch_size), pin_memory=True,
            shuffle=is_train, sampler=None, drop_last=True, collate_fn=custom_collate_fn if is_train and use_dynamic_collate else None
        )
...
def init_data_loaders(to_be_distributed):
    ...
    train_loader = prepare_dataloader(
        MyData(datasets=config.training_set, data_size=None if use_dynamic_collate else config.size, is_train=True),
